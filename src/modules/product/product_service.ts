...
const attachProductMedia = async (
  productId: string,
  payload: {
    mediaId: string;
    isThumbnail?: boolean;
  ...
  if (existingAttachment) {
    throw new AppError(409, 'Media is already attached to this product');
  }
...
const detachVariantMedia = async (variantId: string, mediaId: string) => {
  const variant = await prisma.productVariant.findUnique({
    where: {
      id: variantId,
    },
  ...
  if (!attachment) {
    throw new AppError(404, 'Media is not attached to this variant');
  }
...
const reorderProductMedia = async (
  ...
  await prisma.$transaction(async (tx) => {
    for (const item of payload.items) {
      const attachment = await tx.productMedia.findFirst({
        where: {
          productId,
      ...
      if (!attachment) {
        throw new AppError(
          404,
          `Media ${item.mediaId} is not attached to this product`,
        );
      }
...
const attachAttributeValueMedia = async (
  valueId: string,
  payload: {
    mediaId: string;
    isThumbnail?: boolean;
